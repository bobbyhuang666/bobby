...
router.post('/:id/comment', authMiddleware, validateObjectId('id'), async (req, res) => {
  try {
    ...
    // 安全过滤：防止评论中的注入内容传给 LLM
    const { sanitizeInput, getConfusedReply } = require('../services/securityFilter');
    const sanitized = sanitizeInput(content.trim());
    if (sanitized === null) {
      // 纯注入内容，不传给 AI
      return res.json({
        userComment: content.trim(),
        bobbyReply: getConfusedReply()
      });
    }

    const bobbyEngine = req.app.get('bobbyEngine');
    const result = await bobbyEngine.handleComment(req.userId, req.params.id, sanitized);
