...
// 存储前端本地生成的 Bobby 消息（如"故事钩子"）
// 让后端对话历史包含这些消息，保持上下文连贯
router.post('/sync-bobby-message', authMiddleware, async (req, res) => {
  try {
    ...
    // 安全过滤：防止用户伪造包含注入指令的 Bobby 消息
    if (cleanContent.length > 300) {
      return res.status(400).json({ error: '内容过长' });
    }
    const { isOutOfCharacter } = require('../services/securityFilter');
    if (isOutOfCharacter(cleanContent)) {
      return res.status(400).json({ error: '内容不合法' });
    }
    ...
    console.error('同步 Bobby 消息失败:', err);
