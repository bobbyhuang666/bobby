...
class AndyBridge {
  ...
  /**
   * 获取 Bobby 的内心叙事（替代原来的 getWorldContext 文本拼接）
   *
   * @param {Object} [options]
   * @param {string} [options.userText] - 用户当前消息
   * @param {number} [options.intimacy=0] - 好感度 (0-100)，影响共情强度
   * @returns {string} 叙事文本
   */
  getNarrative(options = {}) {
    if (!this._initialized || !this.andy) return '';

    const agent = this.andy.getAgent(BOBBY_AGENT_ID);
    if (!agent) return '';

    const { userText, intimacy = 0 } = options;
    ...
    if (userText) {
      try {
        ...
        if (rawEffect && Object.keys(rawEffect).length > 0) {
          // 计算共情系数：关系 × 人格 × 状态
          const empathyScale = this._computeEmpathy(agent, intimacy);
   ...
   * 计算共情系数
   *
   * Bobby 对用户情绪的反应强度 = 关系 × 人格 × 状态
   *
   * - 关系（好感度 0-100 → 0-1）：陌生人几乎不受影响，好朋友明显共情
   * - 人格（宜人性 OCEAN.agreeableness）：高宜人性更容易共情
   * - 状态（社交能量）：社交能量低时自顾不暇，共情能力下降
   *
   * @private
   */
  _computeEmpathy(agent, intimacy) {
    // 关系因子：0（陌生）→ 1（信赖）
    // 用 sigmoid 曲线，中间段变化最快
    ...
    // 状态因子：自顾不暇时没精力共情
    // 社交能量低 OR 自己情绪很差 OR 疲劳 → 共情能力下降
    let stateFactor = 1.0;
    if (agent.socialEnergy < 0.3) {
      stateFactor *= 0.5; // 社交能量低，没精力管别人
    }
    if (agent.emotion && agent.emotion.getValence() < -0.15) {
      stateFactor *= 0.6; // 自己也很难过，顾不上
    }
    if (agent.needs && agent.needs.needs.energy < 0.3) {
      stateFactor *= 0.7; // 太累了
    }
